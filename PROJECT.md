...
- **Пересъёмка скриншотов:** `tools/screenshot.py` запускает приложение в
  ...
  сохраняет `site/assets/img/{main,popup}.webp` (1800 px, WebP) и
  `maps.png` (диалог карт) через `QWidget.grab()`. Схема `map://`
  регистрируется до создания QApplication — иначе тайлы не отрисуются.
  После пересъёмки картинки проверяются вручную.
- **Деплой:** статика выкладывается на хостинг заказчика (папка `site/`
  заливается как есть, сборка не нужна). Для справки в `site/README.md` —
  вариант Cloudflare Pages (бесплатно, работает с приватным репозиторием;
  GitHub Pages требует публичный репо или платный Pro) и TODO (публикация
  установщиков в GitHub Releases, иначе ссылки «Скачать» ведут на пустую
  страницу).

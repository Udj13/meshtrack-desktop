...
| `meshtrack/regions.py` | Встроенные регионы (bbox по аэродромам и городам), пользовательские bbox. |
...
- **Встроенные регионы** (координаты `regions.py`):
...
| `innopolis` | Татарстан — Иннополис, Свияжск, аэродром «Куралово» (радиус 20 км) | 55.57190–55.93226 | 48.42445–49.06477 |
...
### Диагностика «не показываются карты» (для поддержки)

**Что попросить у пользователя:** `meshtrack.log` + `config.json` из папки
данных приложения и скриншот менеджера карт (там виден статус каждой карты:
`downloaded`/`partial`/`missing`/`corrupted`/`not_downloaded`/`orphan`).

**Что уже пишется в `meshtrack.log` (по умолчанию INFO, без `--debug`):**

| Симптом в логе | Причина | Лечение |
|---|---|---|
| `Map file not found: <path>` | запись в конфиге есть, файла нет (перенос/чистка/другой диск) | перекачать/перевыбрать карту |
| `No map path for id=<id>` | `active_map_id` не совпадает с записями `maps[]` | конфликт конфига |
| `Tile не PNG (повреждён): … — перекачайте карту` | в MBTiles html-заглушки (оборванная докачка) | перекачать карту |
| `MapStore error` | `.mbtiles` не читается как SQLite (status=corrupted) | перекачать карту |
| `Тайлы не найдены в области: map://…` (INFO, 1 раз на карту) | область просмотра вне bbox/зума карты либо докачка неполная | выбрать/перекачать нужный регион |
| `Область просмотра вне активной карты …: lat=… lon=… zoom=…` | Leaflet-вид не пересекается с bbox/z активной карты | см. пункт выше |
| `WebView loadFinished ok=False` | страница `index.html` не загрузилась вовсе | вне тайлов — копать QtWebEngine |

**Что добавлено для диагностики:**
- при старте в лог пишется сводка всех карт: `Карты: <id> (name), status, file, tiles, z, bbox` + строка `Активная карта: …`;
- JS-консоль QtWebEngine переносится в `meshtrack.log` (JavaScript `console.warn`/`error` — всегда, `log` — только с `--debug`) — так видны `Switching offline map layer to: map://…` из Leaflet;
- в менеджере карт появилась кнопка **«Проверить»**: запускает `MapStore.verify()` (число тайлов, повреждённые/пустые, minmax-zoom) и кладёт отчёт в лог;
- статус-бар: когда центр/зум Leaflet не попадает в bbox/диапазон зумов активной карты, `Карта: <id>` меняется на `Карта: <id> — вне области` (красным) с подсказкой в tooltip.
- `--debug` дополнительно включает потайловые `Tile hit/miss` из `MapSchemeHandler` и raw-строки порта.

**Причины «серого поля» с исправным логом (без ошибок):** вид карты в центре
`[54.4, 45.4], z13` (`app.js`) вне bbox загруженной карты, либо текущий zoom
вне диапазона карты — Leaflet честно запрашивает тайлы, но их в MBTiles нет.

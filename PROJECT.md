...
- **Попап** по клику на маркер: GS (км/ч), высота MSL (м), варио (м/с с ↗/↘),
  заряд %, напряжение В, «обновлено N с назад», флаг SOS.
- **Панель трекеров:** строка = цвет-чип + id + GS + alt + тренд + заряд +
  «last update»; устаревшим (>20 мин) строка добавляется `(!)` и серый текст
  «last update»; клик — центрирует карту на маркере; двойной — вкл/выкл трек.
...
- `Date/Time` → ISO `YYYY-MM-DDTHH:MM:SSZ`; ошибка → `'N/A'`.
...
Из последних точек трекера (окно ≤ 60 сек, минимум 2 точки):

...
## Доработки после фазы 5 (все ✅)

| # | Доработка | Статус |
|---|---|---|
| 5.1 | Время точки = время с устройства (`Date/Time` → `device_ts`, epoch UTC); время приёма — колонка `recv_ts` (миграция `ALTER TABLE` + бэкфилл). Актуальность (возраст, «устарел >20 мин») — от времени устройства; живость канала («Активных: N») — по `recv_ts`; RAM-кэш сортируется по `ts` перед `derive()` | ✅ |
| 5.2 | Подпись «GS» → «Скорость» (панель трекеров, попап маркера, PROJECT.md) | ✅ |
| 5.3 | Таймаут предупреждения о тишине на порту: 5 с → 60 с | ✅ |
| 5.4 | Демо-режим (`--demo` / `MESHTRACK_DEMO=1`): моковые позиции без приёмника (`demo.py`), отдельная БД `meshtrack-demo.db`, тумблер в меню «Данные» | ✅ |
| 5.5 | Тренд в маркере — треугольники `▲`/`▼` (не путать с курсовой стрелкой); видимые треки догружаются при поступлении точек (throttle 5 с); демо-история заполняется по всем трекерам | ✅ |
| 5.6 | Трек показывается по клику на трекер (раньше — только по двойному); двойной клик скрывает; в попапе маркера — ссылка «Показать/скрыть трек» | ✅ |
| 5.7 | Управление картами: статусы (скачана/частично/файл не найден/не подключена/не скачана/повреждена), удаление, докачка, подключение осиротевших файлов; диалог «Управление картами» и метаданные MBTiles (`bbox`, `tile_count_expected`, `complete`) | ✅ |

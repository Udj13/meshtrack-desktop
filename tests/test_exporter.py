...
def test_collect_tracks_period(tmp_path: Path):
    ...
    repo.add_position("boon1", 54.0, 45.0, alt=100, ts=1700000000)
    repo.add_position("boon1", 54.1, 45.1, alt=110, ts=1700000100)
    repo.add_position("boon2", 55.0, 46.0, ts=1700000000)

    tracks = collect_tracks(repo, ts_from=1700000050, ts_to=1700000200)
    assert list(tracks) == ["boon1"]
    assert len(tracks["boon1"]) == 1

    all_tracks = collect_tracks(repo)
    assert sorted(all_tracks) == ["boon1", "boon2"]
...
def test_export_gpx_skips_points_without_coords(tmp_path: Path):
    path = tmp_path / "tracks.gpx"
    tracks = {"boon1": [{"ts": 1.0, "lat": None, "lon": None, "alt": 10}]}
